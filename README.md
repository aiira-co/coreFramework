...
The app contains tow(2) pages which requires a database connection (pratice page and the login page). In the app(coreFramework) folder, there is a coredb.sql file in a folder called '_database file'. Open your database management studio. In XAMPP open the PHPMYADMIN page and create a database called coredb. Import the sql file (coredb.sql) into the database.
...
* CORE:: makes reference the coreFrameworks 'core class' which contains static methods like
  ...
  * getInstance('className') --> for instantiating a core class 'params' and componets.
...
* A more pleasing way would be to use the params class.
  * Create private variable called $params, then instantiate it at the 'constructor()' method
...
* The Framework's core scripts are stored in a folder called 'core' in this  'libraries' folder. DO NOT TOUCH IT; except you know what you are about and plans extend this framework.
* The design folder contains airDesign CSS Framework files which comes by default with the coreFramework.
...
* (mousleave) -- mouse leave event $.mouseleave
...
* adModal-- this causes a pop-up event with ajax loading into ad-modal
